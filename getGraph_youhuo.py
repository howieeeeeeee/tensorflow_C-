...
tf.app.flags.DEFINE_string('image_file','/xhome/tx_zhiwei/inceptionv3_center_loss/test_pic/liang.jpg',"""where is the test image""")
...
tf.app.flags.DEFINE_string('class_file','/xhome/tx_zhiwei/facial_age_images/15_16_labels.txt',"""where is the classes file """)
...
def image_process(image_dir):
  with tf.name_scope('image_processing'):
   ...
   # image = tf.image.decode_jpeg(image, channels=3)
    image = tf.cast(image_dir, tf.uint8)
...
def main(unused_argv):
  with tf.Graph().as_default():
    input_image = tf.placeholder(tf.int32, shape=(None,None,3), name = "input_node")
   # input_image = tf.placeholder(tf.string, name = "input_node")
    ...
    with tf.Session() as sess:
      ...
      if ckpt and ckpt.model_checkpoint_path:
        ...
        return
      tf.train.write_graph(sess.graph, FLAGS.output_graph_dir, "age_graph_test_youhuo.pb", as_text=True)
      result = sess.run(predictions, feed_dict={input_image : FLAGS.image_file})
      print(result)
      result = result.argsort()[0]
      result = list(result)
      result.reverse()
      with open(FLAGS.class_file,'r+') as f:
        ...
        for i in result:
          if not i == 0:
              print(label[i-1])
      tf.train.write_graph(sess.graph, FLAGS.output_graph_dir, "emotions_graph.pb", as_text=True)
