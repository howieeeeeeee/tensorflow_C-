...
def loss(logits, labels, sigma, batch_size=None):
  ...
  if sigma==0:
    ...
    nn = tf.convert_to_tensor(n, dtype=tf.int32)
    
    d_labels = tf.exp(-tf.pow(tf.subtract(s_labels, nn), 2)/2/sigma/sigma)
    d_labels_sum = tf.reduce_sum(d_labels,1)
    d_labels_sum = tf.reshape(d_labels_sum, [batch_size, 1])
    d_labels_sum_temp = d_labels_sum
    for i in range(1,num_classes):
        d_labels_sum = tf.concat([d_labels_sum, d_labels_sum_temp], 1)
    dense_labels = tf.div(d_labels, d_labels_sum)
